...
def tier1_rules(records: list[dict],
    ...
    for r in records:
        ...
        if len(comment) < 10:
            ...
            r.update({"tier": 1, "priority": INCLUDE_HIGH, "should_include": True,
                      "summary": f"{code} event on {r['equip']}",
                      "_t1_reason": f"high_code:{code}"})
            classified.append(r); counts["high_code"] += 1
            log.debug(f"T1 HIGH_CODE  | {r['log_id']} | {code} | {r['equip']}")
...
def tier2_keywords(records: list[dict],
    ...
    for r in records:
        ...
        if m_high:
            ...
            r.update({
                "tier": 2, "priority": INCLUDE_HIGH, "should_include": True,
            ...
            log.debug(f"T2 HIGH_KW    | {r['log_id']} | kw={kw!r} | {comment[:60]}")
...
def build_summary_prompt(groups: dict, shift_label: str) -> str:
    ...
    for level in ("HIGH", "MEDIUM", "LOW"):
        for e in groups[level]:
            ...
            lines.append(
                f"[{level}] {e['ts']} | {e['equip']} | {e['sector']} | {e['summary']}{notif}"
            )
...
def generate_summary(llm: Llama, groups: dict, shift_label: str,
    ...
    if pt > LLM_CONFIG["n_ctx"] * 0.75:
        log.warning(f"Summary prompt at {pt}/{LLM_CONFIG['n_ctx']} tokens — may truncate")
    ...
    print("\n")
    log.info(f"Summary: {len(text)} chars  {elapsed:.1f}s")

    # Dev artifact: save prompt + raw response
...
def main():
    ...
    log.info(f"Final: included={len(included):,}  "
             f"HIGH={len(groups['HIGH'])}  MED={len(groups['MEDIUM'])}  "
