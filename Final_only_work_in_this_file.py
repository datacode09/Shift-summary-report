...
───────────────
  ...
  v7  GBNF grammar (no parse failures); correct two-step pipeline —
      ...
      dynamic n_ctx computed from prompt size before model load;
      hierarchical summarisation for large included sets;
      multi-strategy JSON parser + individual record fallback;
      strip_prompt_echo() for both passes
  v8  config.json + shift_config.json external configuration (Step 1);
      elog_codes.json RAG enrichment — code description + numeric priority
      injected as {{code_context}} in analyze_entry prompt (Step 5);
      token usage tracking per batch and totals (Step 10);
      elog numeric priority (1=CRITICAL,2=HIGH,3=LOW) shown in report
      alongside LLM priority classification
"""
...
def _build_model_config(cfg_section: dict, defaults: dict) -> ModelConfig:
    ...
    defaults contains the legacy flat config values.
    """
    return ModelConfig(
        model_path    = cfg_section.get("model_path",    defaults.get("model_path",    "./mistral-7b-instruct-v0.2.Q4_K_M.gguf")),
        template      = cfg_section.get("template",      defaults.get("template",      "mistral")),
        system_prompt = cfg_section.get("system_prompt", defaults.get("system_prompt", "")),
        n_ctx         = cfg_section.get("n_ctx",         defaults.get("n_ctx",         4096)),
        n_batch       = cfg_section.get("n_batch",       defaults.get("n_batch",       512)),
        n_threads     = cfg_section.get("n_threads",     defaults.get("n_threads",     4)),
        n_gpu_layers  = cfg_section.get("n_gpu_layers",  defaults.get("n_gpu_layers",  0)),
        f16_kv        = cfg_section.get("f16_kv",        defaults.get("f16_kv",        True)),
    )


# ── Shared defaults (legacy flat config support) ──────────────────────────────
_shared_defaults = {
    "model_path":   _cfg.get("model_path",   "./mistral-7b-instruct-v0.2.Q4_K_M.gguf"),
    "template":     _cfg.get("template",     "mistral"),
    "system_prompt": "",
...
# Step 1: analyze_entry — fast small model (Qwen2.5-3B recommended)
STEP1_MODEL: ModelConfig = _build_model_config(
    _cfg.get("step1_model", {}), _shared_defaults
)

# Step 2: executive_summary — quality prose model (Mistral 7B recommended)
STEP2_MODEL: ModelConfig = _build_model_config(
    _cfg.get("step2_model", {}), _shared_defaults
)
